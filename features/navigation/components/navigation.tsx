  ...
  return (
    <nav className="grid grid-cols-3 mx-auto items-center rounded-full border border-border bg-surface/80 p-1.5 backdrop-blur-md">
      ...
      <div className="flex items-center justify-start gap-1">
        <Link
          href="/register"
          className="rounded-full bg-accent px-2 py-2 text-xs text-background transition-all duration-300 hover:bg-accent-light md:px-4 md:text-sm"
        >
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`rounded-full px-2.5 py-2 text-xs transition-all duration-300 md:px-4 md:text-sm ${
                isActive
                  ? "bg-accent/20 text-accent-light"
                  : "text-muted hover:bg-accent/20 hover:text-accent-light"
              }`}
            >
              {item.label}
            </Link>
